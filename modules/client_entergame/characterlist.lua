...
local function tryLogin(charInfo, tries)
  ...
  loadBox = EnterGame.addConnectingBar(brandLoginBox(displayCancelBox(tr('Please wait'), tr('Connecting to game server...'))))
...
function onGameLoginError(message)
  CharacterList.destroyLoadBox()
  errorBox = brandLoginBox(displayErrorBox(tr("Login Error"), message))
...
function onGameLoginToken(unknown)
  ...
  errorBox = brandLoginBox(displayErrorBox(tr("Two-Factor Authentification"), 'A new authentification token is required.\nPlease login again.'))
  errorBox.onOk = function()
    errorBox = nil
    EnterGame.show()
  end
...
function onGameConnectionError(message, code)
  ...
  if (not g_game.isOnline() or code ~= 2) and not errorBox then -- code 2 is normal disconnect, end of file
    ...
    errorBox = brandLoginBox(displayErrorBox(tr("Connection Error"), text))
    errorBox.onOk = function()
      errorBox = nil
      CharacterList.showAgain()
    end
...
function onGameUpdateNeeded(signature)
  CharacterList.destroyLoadBox()
  errorBox = brandLoginBox(displayErrorBox(tr("Update needed"), tr('Enter with your account again to update your client.')))
  errorBox.onOk = function()
    errorBox = nil
    CharacterList.showAgain()
  end  
...
function CharacterList.create(characters, account, otui)
    ...
    for key,value in pairs(characterInfo) do
      ...
      if subWidget then
        if key == 'outfit' then -- it's an exception
          ...
          if key == 'name' then
            -- Keep short names centered (the look you picked); only flip a name to
            -- left-aligned when it's too wide to fit, so its start stays readable.
            pcall(function()
              if subWidget:getTextSize().width > 145 then
                subWidget:setTextAlign(AlignLeftCenter)
              else
                subWidget:setTextAlign(AlignCenter)
              end
            end)
          end
        end
  ...
  local isPremium = not (account.subStatus == SubscriptionStatus.Free and account.premDays < 1)
  -- No pill, no colour difference - Free and Premium render identically (colour from otui).
  accountStatusLabel:setImageSource('')
  accountStatusLabel:setImageBorder(0)
  accountStatusLabel:setPaddingLeft(0)
  accountStatusLabel:setPaddingRight(0)
  if isPremium then
...
function CharacterList.doLogin()
  ...
  if selected then
    ...
    brandLoginBox(displayErrorBox(tr('Error'), tr('You must select a character to login!')))
  end
end
