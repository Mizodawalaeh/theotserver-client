...
function updateFeatures(version)
    ...
    if(version >= 860) then
        ...
        -- Latency ping for 8.60 (stock OTClient only turns it on at >=953). Use the
        -- OTClientV8 EXTENDED ping, not the standard one: this is an otclientv8 server,
        -- which answers the extended ping (the standard ping went unanswered -> bogus ~4000ms).
